...
const SIDE_3: CardId[] = ["S-6", "D-3", "C-5"]; // the drawn 5 went in over the known 9
...
What sets the Cabo version apart are its power cards: 7, 8, 9, 10, jack and queen each carry an ability, remembered at the table with rhymes. Seven or eight, know your fate. Nine or ten, know a friend. Jack or queen, switch between.`,
  ...
  steps: [
    ...
    },
    {
      ...
      title: "Poker face",
      narration:
        "You flip your top-right mystery and... the KING OF DIAMONDS. The only zero in the pack has been asleep in your square the whole time. Not a flicker crosses your face: the peek is private, and a good poker face keeps it that way. The seven goes to the pile, its work done. Take stock of your square: a three, the zero king, an ace and a six. Ten points, and you know every card in it.",
      impact: true,
      apply: (b) =>
        move(flip(b, ["D-K"], true), ["S-7"], "discard", { faceUp: true }),
      highlight: ["D-K"],
      spotlight: ["p-you", "discard"],
      known: YOU_3,
    },
    {
      ...
      narration:
        "Your king settles back face down as the turn passes. Side discards a ten from the stock, nine or ten, know a friend, and peeks the top-left card of YOUR square: the three you slotted on your first turn.",
      uselessNarration:
        "You only see them look. But their ghosts tell the truth: your corner is part of Side’s memory now.",
      apply: (b) =>
        move(flip(b, ["D-K"], false), ["H-10"], "discard", { faceUp: true }),
    ...
    },
    {
      ...
      callout:
        "Nine or ten, know a friend: peek at one card in an opponent’s square.",
      apply: (b) =>
        flip(move(b, ["D-10"], "drawn", { faceUp: true }), ["S-6"], true),
      highlight: ["S-6"],
    ...
    },
    {
      ...
      apply: (b) =>
        move(flip(b, ["S-6"], false), ["D-10"], "discard", { faceUp: true }),
    ...
    },
    {
      ...
      narration:
        "Diagonal draws an ace of clubs, the best card in the pack short of the zero king, and the seven they burned earlier pays off: they KNOW the queen is the rot in their square. The ace slides in over it, twelve points become one, and the queen spins face up onto the pile. No power, no gamble, just a known high card traded for a known low one. The boring play, and the strongest.",
    ...
    },
    {
      ...
      callout:
        "The knock: when a card lands face up on the pile, race to slam its match from any face-down card on the table. Old-schoolers race to knock the table to claim it; we play grab and slam. A wrong knock costs a penalty card.",
      impact: true,
    ...
    },
    {
      ...
      narration:
        "You grab Side’s six and SLAM it onto the pile, your drawn six landing on top to seal it. Side has known that six since their opening peek, but your hand is quicker: payback for the three. Knocking a card out of an opponent’s square has a price: their empty slot is refilled blind from YOUR square, and you hand over the mystery card Across planted on you. Gladly. Your square is down to three cards, every one of them known.",
      callout:
    ...
    {
      id: "false-knock",
      title: "SLAM: the six that is not there",
      narration:
        "Side lunges. They have known the six in their bottom-right corner since the opening peek, and a six just landed face up: easy match, free shed. SLAM. But the table watched you knock that exact six away seconds ago, and the slot now holds the stranger you refilled it with. An eight of spades stares up from the pile. Side’s memory was right; it was just one turn out of date.",
      impact: true,
      apply: (b) => move(b, ["S-8"], "discard", { faceUp: true }),
      highlight: ["S-8"],
      spotlight: ["p-side", "discard"],
      status: { "p-side": "misfire!" },
      known: SIDE_4,
    },
    {
      id: "penalty",
      ...
      narration:
        "A wrong knock is not free. The eight goes back exactly where it was, face down again, though every player at the table has now seen it. Then the sting: Side must draw a penalty card from the stock and slide it into their square without looking. Four cards become five. The square you spend the whole game trying to shrink can be forced to grow.",
      callout:
        "Knock wrong and the card returns to its slot: you draw one penalty card face down into your square for each miss.",
      apply: (b) =>
        move(move(b, ["S-8"], "p-side", { faceUp: false }), ["S-J"], "p-side", {
          faceUp: false,
        }),
      highlight: ["S-J"],
    ...
    },
    {
      ...
      callout:
        "Cabo means last lap: each other player gets exactly one more turn. The hand also ends if a square empties or the stock runs out.",
      impact: true,
      apply: (b) => b,
      spotlight: ["p-you"],
      status: { "p-you": "CABO!" },
    ...
    },
    {
      ...
      narration:
        "Diagonal draws a four of hearts and does not blink: they are holding two more fours, both known by heart since the early turns. A stock draw can always be tossed straight back, so the four hits the pile face up as bait, and SLAM, SLAM, Diagonal crashes their own pair of fours down on top of it. Knocking your own cards needs no refill, so the square simply shrinks: three cards become one lonely ace. Diagonal’s hand is suddenly worth a single point, and your four-point call is beaten.",
      callout:
        "Calling Cabo starts the last lap. It does not freeze the table: knocks and powers still fire.",
      impact: true,
      apply: (b) => {
        let n = move(b, ["H-4"], "discard", { faceUp: true });
        n = move(n, ["S-4"], "discard", { faceUp: true });
        return move(n, ["C-4"], "discard", { faceUp: true });
      },
    ...
    },
    {
      ...
      narration:
        "Across takes the final turn of the hand and draws a two of spades, and they know exactly where its twins live: the two they took off the pile and the two they slotted in themselves. SLAM, SLAM, a double knock of their own. Both twos leave their square, the drawn two lands on top, and the square shrinks from four cards to two: nineteen points down to fifteen. Too late to catch Diagonal, but the instinct is exactly right.",
      callout:
    ...
    },
    {
      ...
      narration:
        "Every square turns face up and the counting starts. Your king of diamonds scores its zero and your call was good for four points, a total that wins most hands. Not this one. Diagonal’s flurry left a single ace on the felt: one point, and the hand is theirs. Across’s late purge lands on 15. And Side’s disaster is complete: the club king, the eight they misfired, and a penalty card that turns out to be a jack pile up to a punishing 42.",
      callout:
        "Totals: Diagonal 1 · You 4 · Across 15 · Side 42. Lowest total wins, and this time the caller is caught.",
      impact: true,
      apply: (b) =>
        SEATS.reduce((acc, z) => flip(acc, cardsInZone(acc, z), true), b),
      spotlight: SEATS,
      status: {
